...
<script>
  ...
  import { getProblem } from '../../../api/student/student_problem';
  ...
  export default {
    ...
    created() {
      ...
      this.student.then(function (res) {
        ...
        _self._getProblem(_self.page, _self.studentInfo.s_speciality);//获取课题
      });//获取学生信息
    },
    methods: {
      ok(){
      ...
      _linkDetail(item) {
        this.$router.push({ path: `/student/problem/${item.p_id}` })
      },
      _getProblem(page, s_speciality) {
        let _self = this;
        getProblem(page, s_speciality).then(function (res) {
          if (res.code == 0) {
      ...
      _clickAdd() {
        this.page++;
        this._getProblem(this.page, this.studentInfo.s_speciality);
      },
    },
    computed: {
      ...mapGetters(['student'])
